		...
		SwingUtilities.invokeLater(new Runnable(){
			public void run(){
				...
				while(done == false){
					...
					if(retVal == null){ // The user closed the startup dialog
						System.exit(0);
					}
					else{
						...
						if (retVal.get(0).equals("Client")){
							...
							if (retVal.get(1).trim().equals("") || retVal.get(2).trim().equals("") || retVal.get(3).trim().equals("") ){
								...
								try {
									...
									client.dispose();
									JOptionPane.showMessageDialog(null, "Error: No room named "+chatroom);
								}
								catch(UserNameException e){
									client.dispose();
									JOptionPane.showMessageDialog(null, "Error: Selecter username already in use.");
								}
